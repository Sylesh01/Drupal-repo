...
(function ($) {
  $(document).ready(function() {
    // Select the phone number field by its ID
    var phoneField = $('#phone-number-field');
    // Select the element where the error message should be displayed
    var nameErrElem = $('.nameerr');
    
    // Initialize the error flag to false
    var hasError = false;
    
    // Add a blur event listener to the phone field
    phoneField.on('blur', function validatePhoneNumber() {
      // Get the phone number value and remove any non-numeric characters
      var phoneNum = $(this).val().replace(/\D/g, '');
      
      // Check that the phone number is 10 digits long
      if (phoneNum.length != 10) {
        // If the phone number is not valid, display an error message
        nameErrElem.html('Please enter a valid 10-digit phone number.');
        // Set the error flag to true
        hasError = true;
      } else {
        // If the phone number is valid, remove the error message
        nameErrElem.html('');
        // Set the error flag to false
        hasError = false;
        // Remove the event listener to prevent multiple validation
        phoneField.off('blur', validatePhoneNumber);
      }
    });
    
    // Add a focus event listener to the phone field
    // phoneField.on('focus', function() {
    //   // If an error has occurred, prevent the field from losing focus
    //   if (hasError) {
    //     $(this).focus();
    //   }
    // });
    
    if($('#edit-email-error').text().trim().length = 0){
      $('#edit-email-error').blur(); 
    }
